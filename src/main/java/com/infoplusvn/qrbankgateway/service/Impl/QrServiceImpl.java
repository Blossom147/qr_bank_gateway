...
public class QrServiceImpl implements QrService {
    ...
    public String showKeyLengthValue(LinkedHashMap<String, String> linkedHashMap) {
        String result = "";
...
//        BankEntity bank = bankRepo.findByBin(qrRequest.getHeaderGW().getBkCd());
        BankEntity bank = bankRepo.findByBin(qrRequest.getHeaderGW().getBkCd());
        log.info(qrRequest.getHeaderGW().getBkCd());
...
//            deCodeQRResponse.getHeaderGW().setBkCd(linkedHashMapQRString.get(QRCodeFormat.CONSUMER_ACCOUNT_INFO.getId() + "." + QRCodeFormat.MEMBER_BANKS.getId() + "." + QRCodeFormat.BNB_ID.getId()));
    ...
    public GenerateQRResponse genAdQR(GenerateAdQR qrRequest) throws IOException, WriterException {
        qrRequest.getHeaderGW().setBrCd(CommonConstant.BRAND_CODE);
        qrRequest.getHeaderGW().setTrnDt(CommonConstant.TRANSACTION_DATE);
        qrRequest.getHeaderGW().setDirection(CommonConstant.DIRECTION_OUTBOUND);
        qrRequest.getHeaderGW().setReqResGb(CommonConstant.REQ_GB);
        qrRequest.getHeaderGW().setRefNo(CommonConstant.REFERENCE_NUMBER);
    ...
    private void createQREntity(GenerateQRRequest qrRequest, GenerateAdQR adQR, String qrType, String qrImage, String qrThemeImage) {
        ...
        //header
        if (qrRequest != null) {
            qrCodeEntity.setBkCd(qrRequest.getHeaderGW().getBkCd());
            qrCodeEntity.setBrCd(qrRequest.getHeaderGW().getBrCd());
            qrCodeEntity.setTrnDt(qrRequest.getHeaderGW().getTrnDt());
            qrCodeEntity.setRefNo(qrRequest.getHeaderGW().getRefNo());
            ...
            qrCodeEntity.setBkCd(adQR.getHeaderGW().getBkCd());
            qrCodeEntity.setBrCd(adQR.getHeaderGW().getBrCd());
            qrCodeEntity.setTrnDt(adQR.getHeaderGW().getTrnDt());
            qrCodeEntity.setRefNo(adQR.getHeaderGW().getRefNo());
        ...
        qrCodeEntity.setDirection(CommonConstant.DIRECTION_OUTBOUND);
